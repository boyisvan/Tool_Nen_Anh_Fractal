...
def calculate_affine_matrix(range_block, domain_block):
    # Phép biến đổi affine bao gồm tỷ lệ
    scale_x = 0.8  # Tỷ lệ theo trục x
    scale_y = 0.8  # Tỷ lệ theo trục y
...
block_size = 64  # Kích thước của mỗi khối
...
for transformation in transformations_loaded:
    ...
    # Sử dụng lại chính phép biến đổi affine đã lưu để tái tạo lại khối phạm vi
    reconstructed_block = apply_transformation(domain_block, domain_block, matrix)
